...
class MockRedis {
  ...
  async zremrangebyscore(key: string, min: number, max: number): Promise<number> {
    ...
    const newMembers = members.filter(item => item.score < min || item.score > max);
...
export class RedisService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit() {
    ...
    const useMock = this.configService.get<string>('NODE_ENV') === 'development' || !redisUrl;

    if (useMock) {
      this.useMock = true;
      this.client = new MockRedis();
      console.log('Using mock Redis client (development mode or no REDIS_URL provided)');
    } else {
